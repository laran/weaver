...
const DEFAULT_STATE_FILE_RELOAD_MS = 500;
...
module.exports = class Weaver {
	...
	constructor(root, options = {}) {
		...
		// Periodically reload state from the state file. By pro-actively reloading it like this we can control how
		// often it gets updated and state can be checked during server requests without having to worry about each
		// request triggering a file read. Doing this here instead of in Server so it's done consistently.
		setInterval(() => {
			let newState = that.getState(true);
			if (that.state !== newState) {
				console.log(`${new Date().toISOString()}: State changed from '${that.state}' to '${newState}'`);
			}
			that.state = newState;
		}, options['stateFileReloadMs']||DEFAULT_STATE_FILE_RELOAD_MS);
	}
	...
	setState(state) {
		...
		// Don't explicitly set the state here. Wait for it to be re-loaded from the state file.
	...
	getState(force = false) {
		if (!!force) {
			return !!fs.existsSync(this.stateFile) ? fs.readFileSync(this.stateFile).toString() : States.NO_FILE;
		}
		return this.state;
	}
